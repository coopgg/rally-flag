...
window.PuzzleHelperData = (function(){
  const TOOLS = [
    ...
    },
    {
      ...
      intro: "Vow of the Disciple has a hidden chest that always drops a guaranteed red border (Deepsight) weapon, claimable once per run. The totem shows 3 of these 9 symbols each week (randomly picked from a larger pool the raid uses overall), and each one has a fixed room somewhere in the raid, marked by a floating \u201cCrux\u201d shard. Shoot the Crux to open the room, then activate the matching symbol inside. Order does matter here in practice: raid encounters are one-way, so once you're past a checkpoint you can't go back for a symbol you missed. The numbers below follow the raid's actual progression, so activate each one as you naturally reach it during a single continuous clear (no checkpoints).",
      runes: [
        { number:1, icon:"puzzle-assets/VotD_06.png", simpleLocation:"Pyramid: Right After the Big Drop", detailedLocation:"Right after the big drop into the Pyramid, in the direction opposite the first encounter. Turn around after landing to find a door with a Crux beside it, then shoot it to reveal the Pyramid symbol." },
        { number:2, icon:"puzzle-assets/VotD_03.png", simpleLocation:"Give: Before Acquisition, After the Symbol Room", detailedLocation:"Before the Acquisition encounter, just after the room with all the interactable symbols. Across from where you entered is a door with a Crux above it, so shoot it to reveal the Give symbol inside." },
    ...
    },
    {
      ...
      positions: [
        ...
        { number: 3, name: "Final Approach (Just Before Nezarec)", simpleLocation: "Hidden room right before the boss room", detailedLocation: "In the branch-and-root climb just before the Nezarec fight, there's a hidden room reached off the main path, with the last set of orbs inside." }
      ]
    },
    {
      ...
      referencePoint: "the first enclosed room at the very start of the raid, with columns of Resonance shapes on the wall. Read the third column from the left, bottom to top: all 5 symbols",
      intro: "Right at the start of Salvation's Edge, before the first encounter, a wall shows columns of Resonance shapes, and the third column from the left is the one that matters, read bottom to top as positions 1 through 5. Positions 1, 3, and 5 (the bottom, middle, and top symbols) are for the bonus red border chest: three real, fixed rooms spread through the raid that are always active every run. Get all three right in a single continuous clear and the chest spawns once the Witness is defeated. Positions 2 and 4 are for a separate raid mod chest (two more fixed rooms), but they only unlock once your fireteam has completed all five of the raid's one-off challenge triumphs; the correct symbol won't do anything there until then. Unlike the red border chest, the mod chest has no weekly limit once it's unlocked.",
      icons: { C: "puzzle-assets/SE_C.png", S: "puzzle-assets/SE_S.png", T: "puzzle-assets/SE_T.png" },
      chestNotes: {
        red: { label: "Red Border Chest", note: "Guaranteed red border weapon. Needs a single continuous clear. Spawns once the Witness is defeated." },
        mod: { label: "Raid Mod Chest", note: "Drops mods you don't already have. Requires all 5 raid challenge triumphs completed first, but no limit on how many times per week after that." }
      },
      positions: [
        { number: 1, chest: "red", name: "Room 1", simpleLocation: "Very start of the raid, in the first large open room", detailedLocation: "Right when you enter the large open room at the start, jump up a bit and follow the path to the left toward a large, well-lit doorway that leads straight into Room 1." },
        { number: 2, chest: "mod", name: "Mod Room 1", simpleLocation: "Also before the first encounter, near the first red border chest room", detailedLocation: "A short distance from the first red border chest room. When you enter a doorframe and need to jump across a chasm, jump to the stairs on your left instead, then crouch down to find a gap in the wall." },
        { number: 3, chest: "red", name: "Room 3", simpleLocation: "Between the 1st and 2nd encounters", detailedLocation: "Between the first and second encounters. After riding the red elevator up, you'll cross large greenish-blue cubes. Keep an eye on the right-side wall for a large, well-lit gap leading into Room 3." },
        { number: 4, chest: "mod", name: "Mod Room 2", simpleLocation: "Before the 3rd encounter, while ascending the monolith", detailedLocation: "While ascending the monolith and avoiding the torrent of falling rectangular slabs, make your way to the second floor. Instead of continuing down the usual path, turn around and jump across to the open room in the inner monolith wall." },
        { number: 5, chest: "red", name: "Room 5", simpleLocation: "Also before the 3rd encounter", detailedLocation: "Also before the third encounter. In the inner room with the rotting tree branches, jump up and to the left, then keep climbing until you reach the conductor plates, the last possible room." }
      ]
    },
    {
      slug: "verity-solver",
